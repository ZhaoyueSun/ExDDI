...
from models.modeling_t5_for_ddi import T5ForDDIClassification
import torch
...
def main():
    ...
    model = T5ForDDIClassification.from_pretrained(
    ...
    def preprocess_function(example):
        ...
        model_inputs["labels"] = [float(1) if label == 'positive' else float(0)]
    ...
    def compute_metrics(eval_preds):
        ...
        predicted_labels = (sigmoid_outputs >= 0.5).long()
        result = metric.compute(predictions=predicted_labels, references=labels.squeeze())
...
if __name__ == "__main__":
    ...
    sys.argv.extend(['configs/molt5_finetune_prediction.json'])
    main()
