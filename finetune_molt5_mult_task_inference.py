...
def main():
    ...
    if training_args.do_predict:
        ...
        logger.info("There is nothing to do. Please pass `do_predict`.")
        return
    ...
    with tokenizer.as_target_tokenizer():
        positive_prefix = tokenizer("positive explanation:").input_ids
        negative_prefix = tokenizer("negative explanation:").input_ids
    ...
    if training_args.do_predict:
        ...
        dataloader = DataLoader(test_dataset, batch_size=training_args.per_device_eval_batch_size, collate_fn=data_collator)
        for batch in tqdm(dataloader):
            ...
            with torch.no_grad():
                ...
                pred = torch.where(pred_prob >= 0.5, 1, 0).cpu().tolist()
                pred_labels += pred
                gold_labels += batch['cls_labels'].cpu().tolist()

                # create decoder prefix
                decoder_prefix = torch.LongTensor([positive_prefix if l else negative_prefix for l in pred]).to(model.device)
                decoder_input_ids = model._shift_right(decoder_prefix)
                batch['decoder_input_ids'] = decoder_input_ids
                cond_outputs = model.generate(**batch, max_length=data_args.max_target_length, num_beams=data_args.num_beams)

                pred_texts += [tokenizer.decode(output, skip_special_tokens=True) for output in cond_outputs]


    # output
    if not os.path.exists(training_args.output_dir):
        os.makedirs(training_args.output_dir)
    with open(os.path.join(training_args.output_dir, "test_preds.txt"), "w") as f:
        f.write("\n".join(pred_texts))

    # binary classification
    cls_metric = evaluate.combine(["accuracy", "recall", "precision", "f1"])
    cls_result = cls_metric.compute(predictions=pred_labels, references=gold_labels)
